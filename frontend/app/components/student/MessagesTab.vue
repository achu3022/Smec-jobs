<template>
  <div class="h-[600px] flex flex-col -mx-4 -mt-4 sm:mx-0 sm:mt-0 bg-white sm:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="flex h-full">
      ...
      <div class="flex-1 flex flex-col bg-white" :class="{ 'hidden md:flex': !activeChat }">
        <div v-if="!activeChat" class="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-50/50">
          ...
          <p class="text-slate-500 max-w-sm">Select a conversation from the sidebar to view messages or start communicating with employers.</p>
        </div>
          ...
          <div class="p-4 border-b border-slate-200 flex justify-between items-center bg-white shrink-0">
            <div class="flex items-center gap-3">
              ...
              <div class="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 text-indigo-700 flex items-center justify-center font-bold">
                {{ groupedMessages[activeChat]?.user?.name?.charAt(0) || 'E' }}
              </div>
              <div>
                <h3 class="font-bold text-slate-900 leading-tight">{{ groupedMessages[activeChat]?.user?.name }}</h3>
                <p class="text-xs text-slate-500 font-medium">Re: <span class="text-slate-700">{{ groupedMessages[activeChat]?.application?.job?.title }}</span></p>
              </div>
            </div>
            <nuxt-link :to="`/jobs/${groupedMessages[activeChat]?.application?.job?.uuid}`" class="text-indigo-600 hover:bg-indigo-50 p-2 rounded-lg text-sm font-bold transition-colors">
              View Job
            </nuxt-link>
          </div>

          <!-- Chat Messages -->
          <div class="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50 custom-scrollbar" id="student-chat-container">
          ...
          <div class="p-4 bg-white border-t border-slate-200 shrink-0">
            <form @submit.prevent="sendReply" class="flex items-end gap-3">
              <div class="flex-1 relative">
                <textarea v-model="newMessage" rows="1" placeholder="Type your reply..." 
...
const scrollToBottom = () => {
  nextTick(() => {
    const el = document.getElementById('student-chat-container')
